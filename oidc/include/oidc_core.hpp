...
struct TokenSet {
	...
	std::string token_type;        // "Bearer", or RFC 8693's "N_A" for a token that is not an access token
	std::string error;
...
//! How a confidential client proves itself at a token endpoint (spec 012). One of:
...
//!   the token endpoint it goes to; `key_id` sets the header's kid, `certificate_pem` adds x5t / x5t#S256;
...
//! Signing needs a TLS build (OpenSSL). Nothing of a key, a certificate or an assertion reaches an error.
struct ClientAuth {
...
//! iat/nbf now, exp now + 60 s - signed RS256 (an RSA key) or ES256 (a P-256 key). An encrypted key is refused.
...
//! and Container Apps: IDENTITY_ENDPOINT + IDENTITY_HEADER (loopback or link-local only); elsewhere IMDS.
...
//! ACTIONS_ID_TOKEN_REQUEST_TOKEN from the job's environment. The token comes back as access_token.
...
//! ("requested_token_type unsupported"): the caller decides whether to ask again without it. A
//! refresh-typed answer is accepted only in Keycloak's shape - an access token and a distinct refresh token
//! beside it; RFC 8693's strict shape (the refresh token itself in access_token, token_type N_A) is refused,
//! so a refresh token is never taken for an access token. The flag may still succeed with no refresh token
//! (an IdP that ignores it): an empty refresh_token means no renewal.
