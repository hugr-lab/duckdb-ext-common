...
- **macOS:** `SecItemAdd` / `SecItemCopyMatching` / `SecItemUpdate` / `SecItemDelete` on a
  ...
  - An unsigned command-line binary gets the file-based login keychain, not the data-protection
...
- **What the OS store protects.** It protects against other users and against a copied disk. It does
  ...
  - on macOS, the item's access list is the application that created it (the DuckDB binary), so
    another binary asks the user first.

  Consumers document this.
- **Never synced:**
  - macOS: `ThisDeviceOnly`;
  - Windows: `CRED_PERSIST_LOCAL_MACHINE`;
  - Linux: the Secret Service's default collection, which is local.
